...
fileXML=open("small_sector_12_SIMLUTATION.xml","w+")
...
for board,elink in zip(boards,elinks):
	...
	if(re.compile("L1DDC").match(board)):
		fileXML.write(templateL1DDC_ADDC() % (counter,board,"L1DDC"))
		l1ddcCounter=l1ddcCounter+1	
	if(re.compile("ADDC").match(board)):
		fileXML.write(templateL1DDC_ADDC() % (counter,board,"ADDC"))
		addcCounter=addcCounter+1	
	if(re.compile("MMFE8").match(board)):
		fileXML.write(templateMMFE8() % (counter,board,"MMFE8"))
