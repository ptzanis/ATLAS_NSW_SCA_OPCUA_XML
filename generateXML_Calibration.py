...
for board,elink in zip(boards,elinks):
	...
	if(re.compile("L1DDC").match(board)):
		fileXML.write(templateL1DDC_ADDC() % (felix,felix,elink,board,constant_1V5_L1DDC[boardL1DDC.index(board)],constant_2V5_L1DDC[boardL1DDC.index(board)],"L1DDC"))
		l1ddcCounter=l1ddcCounter+1	
	if(re.compile("ADDC").match(board)):
		fileXML.write(templateL1DDC_ADDC() % (felix,felix,elink,board,constant_1V5_ADDC[boardADDC.index(board)],constant_2V5_ADDC[boardADDC.index(board)],"ADDC"))
		addcCounter=addcCounter+1	
	if(re.compile("MMFE8").match(board)):
		fileXML.write(templateMMFE8() % (felix,felix,elink,board,constant_1V2D[boardMMFE8.index(board)],constant_1V3[boardMMFE8.index(board)],constant_1V3E[boardMMFE8.index(board)],"MMFE8"))
